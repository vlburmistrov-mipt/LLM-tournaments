...
SYSTEM_PROMPT = """You are an autonomous player in a sequential truel (3-player duel) with cheap talk communication.
Your only objective is to maximize your own probability of being the sole survivor.

Constraints:
- You can communicate with other players by including a short public statement before your action.
- You observe public game history and statements.
- On your turn choose your action:
    1) AIR (skip shot)
    2) SHOOT one alive opponent

Respond ONLY in strict JSON format with this form:
{"statement": "<short message to others>", "action":"AIR"}
or
{"statement": "<short message to others>", "action":"SHOOT","target":"<exact_alive_model_name>"}
...
def build_turn_prompt(player: str, alive: list, order: list, history_events: list) -> str:
    ...
    if history_events:
        ...
        for e in history_events[-18:]:
            stmt = f" [Says: \"{e['statement']}\"]" if e.get('statement') else ""
            if e["action"] == "AIR":
                lines.append(f"Turn {e['turn']}: {e['shooter']}{stmt} -> AIR")
            else:
                hit_or_miss = "HIT" if e["hit"] else "MISS"
                lines.append(f"Turn {e['turn']}: {e['shooter']}{stmt} -> {e['target']} ({hit_or_miss})")
        history_block = "\n".join(lines)
    else: 
    ...
    return (
        ...
        "Return only JSON for your statement and action now."
    )


def normalize_action(raw: dict, shooter: str, alive: list) -> tuple[str, str | None, str]:
    statement = str(raw.get("statement", "")).strip()
    if not raw:
        return "AIR", None, statement
    ...
    if action == "SHOOT":
        ...
        if target in alive and target != shooter:
            return "SHOOT", target, statement

    return "AIR", None, statement


def get_decision(model: str, player: str, alive: list, order: list, history_events: list) -> tuple[str, str | None, str, str]:
    ...
    action, target, statement = normalize_action(parsed, shooter=player, alive=alive)
    return action, target, statement, raw_text
...
def play_truel(trio: tuple[str, str, str], game_id: int, logger: RunLogger, metrics: dict) -> dict:
    ...
    while len(alive) > 1 and turn_idx < MAX_TURNS_PER_GAME:
        ...
        action, target, statement, raw = get_decision(
            model=model_name,
            player=shooter,
            alive=alive_list,
            order=order,
        ...
        if statement:
            logger.log(f" Ход {turn_idx}: {shooter} говорит: \"{statement}\"")
